...
else if((height<pos_top||height>pos_bot)){}})});$timeout(function(){p(block_height);$scope.block_height=block_height;$scope.vis_clips=vis_clips;$scope.is_scroll=!1},700)};$scope.doRefresh=function(){if($state.includes("search-clips")){$scope.$broadcast('scroll.refreshComplete');return !1}
...
if(appId){openFB.init({appId:appId})}
$scope.fbLogin=function(){openFB.login(function(response){if(response.status==='connected'){openFB.api({path:'/me',success:function(data){data.is_fb=!0;$scope.signIn(data,"social")}});p('Facebook login succeeded, got access token: '+response.authResponse.token)}
...
if(ionic.Platform.isIOS()){return{location:'yes',clearcache:'no',clearsessioncache:'no'}}}});angular.module('bazaarr').service('userPicture',function($timeout,$ionicLoading,$ionicPopup,$cordovaCamera,UserService,DeviceAdapterService,AccountService,ToastService,HttpService){this.popup=null;this.imgPopup=function(scope){if(AccountService.account.uid!=UserService.user.uid){return !1}
...
scope.image_src=scope.file.file;scope.file.fid=null;self.uploadImage(scope);self.photo_source_popup.close()}},{text:'Cancel',onTap:function(){self.photo_source_popup.close()}}]}
$timeout(function(){self.photo_source_popup=$ionicPopup.show(opts)})};this.closeImagePopup=function(scope){this.photo_source_popup.close()}
...
timer=$timeout(function(){if(search.search_api_views_fulltext&&search.search_api_views_fulltext.length>=1){if($state.current.name=='search-users'){SearchService.userSearch(search.search_api_views_fulltext).then(function(data){$scope.users=data.data})}else if($state.current.name=='search-collections'){SearchService.collectionSearch(search.search_api_views_fulltext).then(function(data){$scope.collections=data.data})}else if($state.current.name=='search-clips'){SearchService.params=search;SearchService.load().then(function(data){$scope.$parent.$parent.clips=ClipsService.prepare(data.data)})}else if($state.current.name=='search-tags'){SearchService.params=search;SearchService.tagSearch(search.search_api_views_fulltext,30).then(function(data){if(data.data.length>0){for(var i=0;i<data.data.length;i++){data.data[i].name=data.data[i].name.substr(1,data.data[i].name.length)}}
...
$scope.tags=data.data});SearchService.userSearch(search.search_api_views_fulltext,SEARCH_USERS_COUNT).then(function(data){$scope.users=data.data});SearchService.collectionSearch(search.search_api_views_fulltext,SEARCH_COLLS_COUNT).then(function(data){$scope.collections=data.data});$scope.is_load_more=!1;SearchService.params=search;SearchService.load().then(function(data){$scope.clips=ClipsService.prepare(data.data)})}
...
return decodeURIComponent(this.params.search_api_views_fulltext)};this.isSearch=function(){if($state){return $state.includes('search-clips')}
